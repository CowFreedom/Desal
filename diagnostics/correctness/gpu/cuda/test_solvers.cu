	...
	namespace cuda{
		...
		bool test_mg_vc_poisson_2D_f32_zero_B(int n, int reps, char* error_message=nullptr){
			for (int i=1;i<=reps;i++){	
				...
				mg_vc_poisson_2D_device<float,float2,char>(alpha, gamma,eta, 1, n, B, pitch_b, U, pitch_u);
				//desal::cuda::mg_vc_poisson_2D_device<float, float2,std::ostream>(alpha, gamma,eta, 1, n, B, pitch_b, U, pitch_u,&std::cout);
					
			//	reduce_sum_of_squares_poisson_field_residual_device<float,float2>(alpha,beta,1, n,U,pitch_u, B, pitch_b, r);
			//	
