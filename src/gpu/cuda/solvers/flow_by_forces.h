...
namespace desal{
	...
	namespace cuda{
		...
		__global__
		void k_add_forces(F dt, int boundary_padding_thickness, int m, int k,  F2* A, size_t pitch_a, F2* Fs, size_t pitch_f, F2* C, size_t pitch_c){
			...
			Fs=(F2*)((char*)Fs+boundary_padding_thickness*pitch_f)+boundary_padding_thickness;
		...
		void add_forces(F dt, int boundary_padding_thickness, int m, int k, F2* A_d, size_t pitch_a, F2* F_d, size_t pitch_f, F2* C_d, size_t pitch_c){
			...
			k_add_forces<<<blocks,threads>>>(dt, boundary_padding_thickness, m, k, A_d, pitch_a, F_d, pitch_f, C_d, pitch_c);				
		}
	}
}